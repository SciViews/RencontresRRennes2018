...
![](figures/study-function-clones-in-CRAN.png)
...
Trois philosophies différentes principales (cf. Amélia McNamara) : **dollar** & **formule** (R base) _versus_ **Tidyverse**
...
**Règle `chart` #2: formule `f_aes()` se substitue à `aes()`.**
...
- Les formules "étendues" sont aussi utilisables avec `ggplot2` _(fini)_
...
### Création explicite d'un objet `flow` : permet d'inclure d'autres variables {.fragile}
...
### Utilisation plus transparente des `quosure`s dans `flow` {.fragile}
...
Présentation et version plus détaillée sous forme de **R Notebook** disponible à https://github.com/SciViews/RencontresRRennes2018
